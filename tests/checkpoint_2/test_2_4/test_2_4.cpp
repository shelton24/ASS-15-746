...
int main(int argc, char *argv[])
{
  ...
  if((ret_status == FAILURE) || (!ssd->is_valid(lba, address))) {
      fprintf(log_file_stream, "Error writing LBA 0 first time\n");
      failed(ssd);
  }
  ...
  lba = 0;
  fprintf(log_file_stream, "----------------\nReading LBA %lu\n", lba);
  ssd -> event_arrive(READ, lba, 1, time++, &ret_status, address_first);
  if((ret_status == FAILURE) || (!ssd->is_valid(lba, address_first))) {
      fprintf(log_file_stream, "Error reading LBA 0 first time\n");
      failed(ssd);
  }
  sprintf(addr_str, "[%u,%u,%u,%u,%u]", address_first.package, address_first.die,
        address_first.plane, address_first.block, address_first.page);
  fprintf(log_file_stream, "LBA %lu -> PBA %s\n", lba, addr_str);  
  ...
  // read LBA0 back again
  lba = 0;
  fprintf(log_file_stream, "----------------\nReading LBA %lu\n", lba);
  ssd -> event_arrive(READ, lba, 1, (++time), &ret_status, address_last);
  if((ret_status == FAILURE) || (!ssd->is_valid(lba, address_last))) {
      fprintf(log_file_stream, "Error reading LBA0 second time\n");
      failed(ssd);
  }
  sprintf(addr_str, "[%u,%u,%u,%u,%u]", address_last.package, address_last.die,
          address_last.plane, address_last.block, address_last.page);
  fprintf(log_file_stream, "LBA %lu -> PBA %s\n", lba, addr_str);
